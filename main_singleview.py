...
def main(**args):
    ...
    for idx in range(len(dataset_obj)):
        ...
        # prepare output path
        curr_mesh_fn = osp.join(output_folder, 'meshes', demo_dsname,
                                demo_subids, demo_seqname,
                                f'smpl_{curr_idx:03d}.obj')
        curr_result_fn = osp.join(output_folder, 'results', demo_dsname,
                                  demo_subids, demo_seqname,
                                  f'smpl_{curr_idx:03d}.npz')
        ...
        curr_gt_depths_fn = osp.join(output_folder, 'gt_depths', demo_dsname,
                                     demo_subids, demo_seqname,
                                     f'depth_{curr_idx:03d}.obj')
