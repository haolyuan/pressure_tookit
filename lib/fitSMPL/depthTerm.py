...
class DepthTerm(nn.Module):
    def __init__(self,
        ...
        self.dtype=dtype
        self.device=device
        hand_ids = np.loadtxt(f'{essential_root}/hand_ids.txt').astype(np.int32)
        self.valid_verts = np.ones(6890)
        self.valid_verts[hand_ids] = 0

        self.renderer = modelRender(cam_intr,img_W,img_H)

        self.foot_ids_surfaces = np.load(f'{essential_root}/foot_related/foot_ids_surfaces.npy').tolist()
    ...
    def findCorrs(self,depth_vmap=None,depth_nmap=None,
        ...
        live_mesh = trimesh.Trimesh(vertices=live_verts,
                        faces=self.model_faces, process=False)
        ...
        tar_normals_smpl2depth = depth_nmap[indices_smpl2depth.reshape(-1)].reshape(-1, icp_near_size, 3)
        ...
        dists_depth2smpl, indices_depth2smpl = kdtree_depth2smpl.query(depth_vmap, k=icp_near_size)
        
        tar_normals_depth2smpl = normal_src[indices_depth2smpl.reshape(-1)].reshape(-1, icp_near_size, 3)
        cosine_depth2smpl = np.einsum('ijk,ik->ij', tar_normals_depth2smpl, depth_nmap)
        ...
        if self.save_obj:
            ...
            with open('debug/corrs_depth2smpl%d.obj', 'w+') as fp:
                for vi in range(depth_vmap.shape[0]):
                    fp.write('v %f %f %f\n' % (depth_vmap[vi, 0], depth_vmap[vi, 1], depth_vmap[vi, 2]))
                    fp.write('v %f %f %f\n' % (tar_verts[vi, 0], tar_verts[vi, 1], tar_verts[vi, 2]))
                for li in range(depth_vmap.shape[0]):
                    fp.write('l %d %d\n' % (2*li + 1, 2*li+2))
        return smpl_visible_idx, indices_corr_smpl2depth, indices_corr_depth2smpl

    def forward(self, depth_vmap=None, depth_nmap=None,
                      live_verts=None):
        """_summary_

        Args:
            depth_vmap (torch.tensor, optional): depth cloud. Defaults to None.
            depth_nmap (torch.tensor, optional): depth cloud norm. Defaults to None.
            live_verts (torch.Size([6890, 3]), optional): smpl surface vertices. Defaults to None.
