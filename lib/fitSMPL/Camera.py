...
class RGBDCamera(nn.Module):
    ...
    def __init__(self, basdir, dataset_name, sub_ids, seq_name):
        ...
        cali_path = osp.join(basdir, self.dataset_name, self.sub_ids,
                             self.seq_name, 'calibration.npy')
        # intr: fx,fy,cx,cy,
        self.cIntr_cpu, self.dIntr_cpu, self.d2c_cpu =\
            self.loadCalibrationFromNpy(cali_path)
        cIntr = nn.Parameter(
            torch.tensor(self.cIntr_cpu, dtype=self.dtype),
            requires_grad=False)
        self.register_parameter('cIntr', cIntr)
        dIntr = nn.Parameter(
            torch.tensor(self.dIntr_cpu, dtype=self.dtype),
            requires_grad=False)
        self.register_parameter('dIntr', dIntr)
        d2c = nn.Parameter(
            torch.tensor(self.d2c_cpu, dtype=self.dtype), requires_grad=False)
