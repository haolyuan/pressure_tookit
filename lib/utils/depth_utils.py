...
# ============================Floor============================
def normalizeFloor(floor_normal,
                   floor_trans=([0, 0, 0]),
...
def calculateFloorNormal(depth_path, xy, fx, fy, cx, cy, save_path=None):
    ...
    # trimesh.Trimesh(vertices=pointCloud.reshape(-1, 3),
    #                 process=False).export('debug/pointCloud.obj')
    depth_slices = (pointCloud[xy[0]:xy[1], xy[2]:xy[3], :]).reshape([-1, 3])
    # trimesh.Trimesh(vertices=depth_slices,
    #                 process=False).export('debug/depth_slice.obj')
    A = depth_slices
    ...
    depth_slices_rot = (floor_rot[:3, :3] @ depth_slices.T +
                        floor_rot[:3, 3].reshape([3, 1])).T
    floor_trans = np.array([
        -np.median(depth_slices_rot[:, 0]), -np.median(depth_slices_rot[:, 1]),
        -np.median(depth_slices_rot[:, 2])
    ])
    depth2floor = normalizeFloor(
        floor_normal=floor_normal, floor_trans=floor_trans)
    ...
    depth_slices_RT = (floor_rot[:3, :3] @ pointCloud.T +
                       floor_trans.reshape([3, 1])).T
    trimesh.Trimesh(
        vertices=depth_slices_RT,
        process=False).export('debug/depth_slice_rot.obj')
...
if __name__ == '__main__':
    ...
    os.makedirs(
        f'/data/yuanhaolei/PressureDataset_label/{dataset_name}/{sub_id}',
        exist_ok=True)

    cali_data = dict(
        np.load(
            '/data/PressureDataset/' + f'{dataset_name}/{sub_id}/{seq_name}' +
            '/calibration.npy',
    ...
    floor_normal, floor_trans, depth2floor = calculateFloorNormal(
        '/data/PressureDataset/' + f'{dataset_name}/{sub_id}/{seq_name}' +
        '/depth/003.png', [512, 565, 181, 486],
        fx=cali_data['depth_Intr']['fx'],
        fy=cali_data['depth_Intr']['fy'],
        cx=cali_data['depth_Intr']['cx'],
        cy=cali_data['depth_Intr']['cy'],
        save_path='/data/yuanhaolei/PressureDataset_label/' +
        f'{dataset_name}/{sub_id}/floor_{sub_id}.npy')
