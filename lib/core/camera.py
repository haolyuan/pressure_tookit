...
def create_camera(basdir,
                 ...
                 sub_ids,
                 seq_name,):
    calibration_path = osp.join(basdir, 'images', dataset_name, sub_ids, seq_name,'calibration.npy')
    floor_path = osp.join(basdir, 'annotations', dataset_name, 'floor_info', 'floor_'+sub_ids+'.npy')
    ...
    if check_path_existence(floor_path) == Existence.FileNotExist:
        ...
        raise FileExistsError    
    return RgbdCamera(cali_path=calibration_path,
...
class RgbdCamera(nn.Module):
    def __init__(self,
                cali_path, floor_path,
                dtype=torch.float32):
        super(RgbdCamera, self).__init__()    

        self.dtype = dtype
        # load data for color&depth transfer
        self.cIntr_cpu, self.dIntr_cpu, depth2color_cpu = loadCalibrationFromNpy(cali_path)
        # load data for depth&floor transfer
        floor_trans_cpu, floor_normal_cpu, depth2floor_cpu = loadFloorFromNpy(floor_path)
        
        
        # intr: fx,fy,cx,cy
        cIntr = torch.tensor(self.cIntr_cpu,dtype=self.dtype)
        self.register_parameter('cIntr', nn.Parameter(cIntr, requires_grad=False))
        dIntr = torch.tensor(self.dIntr_cpu, dtype=self.dtype)
        self.register_parameter('dIntr', nn.Parameter(dIntr, requires_grad=False))

        # floor data
        floor_trans = nn.Parameter(torch.tensor(floor_trans_cpu, dtype=self.dtype), requires_grad=False)
        self.register_parameter('floor_trans', floor_trans)
        floor_normal = nn.Parameter(torch.tensor(floor_normal_cpu, dtype=self.dtype), requires_grad=False)
        self.register_parameter('floor_normal', floor_normal)
        
        # 3d transform
        depth2color = nn.Parameter(torch.tensor(depth2color_cpu, dtype=self.dtype), requires_grad=False)
        self.register_parameter('depth2color', depth2color)
        depth2floor = nn.Parameter(torch.tensor(depth2floor_cpu, dtype=self.dtype), requires_grad=False)
        self.register_parameter('depth2floor', depth2floor)        
        floor2color_cpu = depth2color_cpu @ np.linalg.inv(depth2floor_cpu)
        floor2color = nn.Parameter(torch.tensor(floor2color_cpu, dtype=self.dtype), requires_grad=False)
    ...
    def tranform3d(self, points, type):
        if not torch.is_tensor(points):
            points = torch.tensor(points, dtype=self.dtype).to(self.depth2color.device)
    ...
    def preprocessDepth(self, depth_map, mask_map):
        ...
        if dn_valid_cpu is not None:
            dn_valid = torch.from_numpy(dn_valid_cpu).to(device=dv_floor.device, dtype=self.dtype)
            dn_floor = (self.depth2floor[:3, :3] @ dn_valid.T).T
        
        # return depth_floor, depth_normal
        return torch.from_numpy(dv_valid_cpu).to(device=dv_floor.device, dtype=self.dtype),\
            torch.from_numpy(dn_valid_cpu).to(device=dv_floor.device, dtype=self.dtype),\
            dv_floor, dn_floor
