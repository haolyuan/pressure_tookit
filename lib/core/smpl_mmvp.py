...
from smplx.utils import ModelOutput, Struct, to_np, to_tensor
...
from lib.data.constants_spin import (  # # noqa: yapf
    FOOT_IDS_SMPLL, FOOT_IDS_SMPLR, JOINT_MAP, JOINT_NAMES,
)
...
class SMPL_MMVP(nn.Module):
    ...
    def __init__(self,
        ...
        j_regressor = to_tensor(
            to_np(data_struct.J_regressor), dtype=self.dtype)
        ...
        baby_mesh = load_mesh(
            f'{essential_root}' + '/smplify_essential/' +
        ...
        self.register_buffer('J_regressor_extra',
                             torch.tensor(J_regressor_extra, dtype=self.dtype))

        # load joints mapping to remap smpl joints
        joints_ids = [JOINT_MAP[i] for i in JOINT_NAMES]
        self.register_buffer('joint_map',
                             torch.tensor(joints_ids, dtype=torch.long))
        ...
        self.foot_ids_smplL, self.foot_ids_smplR =\
            FOOT_IDS_SMPLL, FOOT_IDS_SMPLR
    ...
    def init_param(self, stage):
        ...
        # Add extra shape param and weight to
        # control body scale for different ages
        betas = torch.zeros([1, self.num_betas + 1],
                            dtype=self.dtype)  # scale and betas
        betas[:, 0] = 1
        self.register_parameter('betas', nn.Parameter(betas))
        self.betas.requires_grad = True if stage == 'init_shape' else False

        model_scale_opt = torch.tensor([1.0], dtype=self.dtype)
        self.register_parameter('model_scale_opt',
                                nn.Parameter(model_scale_opt))
        self.model_scale_opt.requires_grad = True\
            if stage == 'init_shape' else False

        # init body pose, global rot and transl
        body_pose = torch.zeros([1, self.NUM_JOINTS * 3], dtype=self.dtype)
        self.register_parameter('body_pose',
                                nn.Parameter(body_pose, requires_grad=True))
        transl = torch.zeros([self.bs, 3], dtype=self.dtype)
        self.register_parameter('transl',
                                nn.Parameter(transl, requires_grad=True))
    ...
    def update_shape(self):
        # update v_template
        self.v_template = (
            (self.smpl_v_template * self.model_scale_opt) +
            (1.0 - self.model_scale_opt) * self.smil_v_template).to(
                self.shapedirs.device)

        blend_shape = torch.einsum('bl,mkl->bmk',
                                   [self.betas[:, 1:], self.shapedirs])
    ...
    def init_plane(self):
        assert self.v_shaped is not None, 'must init smpl shape first'

        # separate foots into two part
        self.foot_front_r_ids = [i for i in range(36)
                                 ] + [36, 41, 42, 47, 48, 49]
        self.foot_back_r_ids = [60, 61, 62, 69, 70, 71
                                ] + [i for i in range(72, 96)]
        ...
        v_smpl_foot_L, v_smpl_foot_R =\
            self.v_shaped[0][self.foot_ids_smplL, :], \
            self.v_shaped[0][self.foot_ids_smplR, :]
        ...
        # offset_L, offset_R =\
        #     (y_smpl_foot_L - floor_L).tolist(), \
        #     (y_smpl_foot_R - floor_R).tolist()
        ...
        self.plane_L_init[:, 0], self.plane_L_init[:, 2] =\
            v_smpl_foot_L[:, 0], v_smpl_foot_L[:, 2]
        temp = [
            floor_L.unsqueeze(0) for i in range(self.plane_L_init.shape[0])
        ]
        self.plane_L_init[:, 1] = torch.concat(temp)

        self.plane_R_init[:, 0], self.plane_R_init[:, 2] =\
            v_smpl_foot_R[:, 0], v_smpl_foot_R[:, 2]
        temp = [
            floor_R.unsqueeze(0) for i in range(self.plane_R_init.shape[0])
        ]
        self.plane_R_init[:, 1] = torch.concat(temp)

        self.foot_ids_back_smplL = np.array(
            self.foot_ids_smplL)[self.foot_back_l_ids].tolist()
        self.foot_ids_back_smplR = np.array(
            self.foot_ids_smplR)[self.foot_back_r_ids].tolist()
        self.foot_ids_front_smplL = np.array(
            self.foot_ids_smplL)[self.foot_front_l_ids].tolist()
        self.foot_ids_front_smplR = np.array(
            self.foot_ids_smplR)[self.foot_front_r_ids].tolist()
    ...
    def update_pose(self, body_pose=None):
        ...
        rot_mats = batch_rodrigues(full_pose.view(-1,
                                                  3)).view([self.bs, -1, 3, 3])
        J_transformed, A = batch_rigid_transform(
            rot_mats, self.J_shaped, self.parents, dtype=self.dtype)
        ...
        return SMPLMMVPOuput(
            ...
            foot_ids=[
                self.foot_ids_back_smplL, self.foot_ids_front_smplL,
                self.foot_ids_back_smplR, self.foot_ids_front_smplR
            ])
    ...
    def update_plane(self, T):
        ...
        homogen_coord_back_l = torch.ones(
            [1, self.plane_back_L_init.shape[0], 1],
            dtype=self.dtype).to(device)

        v_plane_homo_back_l = torch.cat(
            [self.plane_back_L_init.expand(1, -1, -1), homogen_coord_back_l],
            dim=2)
        v_plane_back_l = torch.matmul(
            T_back_l, v_plane_homo_back_l.unsqueeze(-1)).squeeze(-1)[:, :, :3]

        # back right foot plane
        homogen_coord_back_r = torch.ones(
            [1, self.plane_back_R_init.shape[0], 1],
            dtype=self.dtype).to(device)
        v_plane_homo_back_r = torch.cat(
            [self.plane_back_R_init.expand(1, -1, -1), homogen_coord_back_r],
            dim=2)
        v_plane_back_r = torch.matmul(
            T_back_r, v_plane_homo_back_r.unsqueeze(-1)).squeeze(-1)[:, :, :3]

        # front right foot plane
        homogen_coord_front_r = torch.ones(
            [1, self.plane_front_R_init.shape[0], 1],
            dtype=self.dtype).to(device)
        v_plane_homo_front_r = torch.cat(
            [self.plane_front_R_init.expand(1, -1, -1), homogen_coord_front_r],
            dim=2)
        v_plane_front_r = torch.matmul(
            T_front_r,
            v_plane_homo_front_r.unsqueeze(-1)).squeeze(-1)[:, :, :3]

        # front left foot plane
        homogen_coord_front_l = torch.ones(
            [1, self.plane_front_L_init.shape[0], 1],
            dtype=self.dtype).to(device)
        v_plane_homo_front_l = torch.cat(
            [self.plane_front_L_init.expand(1, -1, -1), homogen_coord_front_l],
            dim=2)
        v_plane_front_l = torch.matmul(
            T_front_l,
            v_plane_homo_front_l.unsqueeze(-1)).squeeze(-1)[:, :, :3]

        v_plane = torch.concat(
