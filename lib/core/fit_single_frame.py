...
def fit_single_frame(img,
    ...
    trimesh.Trimesh(vertices=dv_floor.detach().cpu().numpy()).export(output_gt_depth_fn)
    ...
    if stage == 'init_shape':
        ...
        # # 
        # params_dict['body_pose'][:, 17*3:17*3+3] = torch.zeros((1,3))
        # params_dict['body_pose'][:, 18*3:18*3+3] = torch.zeros((1,3))
        ...
        pre_pose = params_dict['body_pose'] # params_dict['body_pose'], optionally
    else:
        # TODO: add global rot and transl
        params_dict['body_pose'] = torch.from_numpy(init_pose).to(dtype=dtype, device=device)
        params_dict['betas'] = torch.from_numpy(init_shape).to(dtype=dtype, device=device)
        params_dict['model_scale_opt'] = torch.from_numpy(init_scale).to(dtype=dtype, device=device)
        params_dict['global_orient'] = torch.from_numpy(init_global_rot).to(dtype=dtype, device=device)
        params_dict['transl'] = torch.from_numpy(init_transl).to(dtype=dtype, device=device)
        pre_pose = params_dict['body_pose'] # essentially when tracking smoothly
        

        # # add extra pose for tracking
        # extra_data_fn = '/data/yuanhaolei/PressureDataset_annotations/temp/20230422/S12/MoCap_20230422_150210/MoCap_20230422_150210_cliff_hr48.npz'
        # extra_data = dict(np.load(extra_data_fn).items())
        # extra_data['pose'] = extra_data['pose'][:, 3:]
        # params_dict['body_pose'][:, 16*3:16*3+3] = torch.from_numpy(extra_data['pose'][:, 16*3:16*3+3])
        # params_dict['body_pose'][:, 18*3:18*3+3] = torch.from_numpy(extra_data['pose'][:, 18*3:18*3+3])
        # params_dict['body_pose'][:, 13*3:13*3+3] = torch.from_numpy(extra_data['pose'][:, 13*3:13*3+3])
        # # # # params_dict['body_pose'][:, 3*3:3*3+3] = torch.from_numpy(extra_data['pose'][:, 3*3:3*3+3])
